...
class DataLoader:
    ...
    def download_data(self):
        ...
        # Deixar as colunas df['Date'] e df['Close']
        df = df[["Date", "Close"]]
        ...
        if "Close" not in df.columns:
            raise ValueError(
                "A coluna 'Close' não foi encontrada no DataFrame após o reset do índice."
            )

        # Adicionar indicadores técnicos antes de salvar os dados
    ...
    def load_data(self):
        ...
        if os.path.exists(self.filepath + ".csv"):
            df = pd.read_csv(self.filepath + ".csv", sep=";")
